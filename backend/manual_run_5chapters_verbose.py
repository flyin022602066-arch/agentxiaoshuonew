...
async def monitor_task(task_id: str):
    ...
    while True:
        ...
        snapshot = (task.get("status"), task.get("progress"), task.get("current_stage"))
        if snapshot != last:
            print(f"  [任务进度] status={task.get('status')} progress={task.get('progress')} stage={task.get('current_stage')}")
            last = snapshot
...
async def main():
    ...
    settings = {
        ...
        "creative_settings": {"style_id": "default", "strength": "medium", "techniques": []},
    }
    ...
    for index, outline in enumerate(outlines, start=1):
        ...
        print(f"大纲：{outline}")
        task_id = f"manual-test-{index}-{uuid4().hex[:8]}"
        get_task_manager().create_task(task_id=task_id, task_type="manual_test", metadata={"chapter_num": index, "novel_id": novel_id})
        create_task = asyncio.create_task(
            service.create_chapter_workflow(
                {
                    "novel_id": novel_id,
                    "chapter_num": index,
                    "outline": outline,
        ...
        monitor = asyncio.create_task(monitor_task(task_id))
